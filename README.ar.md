...
# XXD Panel 193｜ألوان مائية هادئة على قاعدة منخفضة
...
ألوان مائية هادئة على قاعدة منخفضة
...
- `design-only`: تعرض اللوحة الكاملة ترجمة Panel 193 التصميمية فقط، وتبقى الصورة مرجعاً غير ظاهر.
...
بعد التثبيت أعد تشغيل جلسة Agent ثم استدعِ `$xxd-panel-193`. ويمكن إضافة `--global --agent codex --yes` للتثبيت على مستوى المستخدم.

```text
/xxd-panel-193 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ar-SA
...
راجع [SKILL.md](SKILL.md) لعقد التشغيل الكامل، ومهايئ التشغيل [بالإنجليزية](references/xxd-panel-193-prompt.en.md) أو [بالصينية](references/xxd-panel-193-prompt.zh-CN.md).
